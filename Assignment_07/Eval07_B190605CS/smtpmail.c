...
void *handle_client(void *arg){
    ...
    while(1){
            char buffer[MAX_BUFFER], body[MAX_BUFFER], from[50], to[50], subject[50];
            char *data;
            int l, n, j=0;
           ...
           printf("New mail Recieved...\n");
           get_data(data, "From", from);
           //printf("%s\n", from);
           get_data(data, "To", to);
           get_data(data, "Subject", subject);
           get_data(data, "Body", body);
           printf("Mail::From: %s ->To: %s\n", from , to);
           
           //Verify mail
           if(!(verify_format(from))){
              char *prompt = "Invalid sender mail\n";
              send_packet(C->sock_fd, prompt, strlen(prompt));
              continue;
           }
           
           if(!(verify_format(to))){
              char *prompt = "Invalid Reciever mail\n";
              send_packet(C->sock_fd, prompt, strlen(prompt));
              continue;
           }
           
           if((j= verify_reciepent(to, C->D, C->users)) == -1){
              printf("Reciepent not Found\n");
              char *prompt = "Invalid Mail";
              send_packet(C->sock_fd, prompt, strlen(prompt));
              continue;
           }
           
           char mailbox_file[100], time_str[50];
           sprintf(mailbox_file, "%s/mymailbox.mail", C->D[j].uname);
           FILE *mailbox = fopen(mailbox_file, "a");
           ...
           fprintf(mailbox, "Recieved: %s\n", time_str);
           fprintf(mailbox, "%s", body);
           fclose(mailbox);
           
           char *res = "EMAIL_SENT";
           printf("New mail sent from %s to %s\n", from, to);
...
int main(int argc, char *argv[]){
   ...
   printf("Initiating SMTP Mail server.\n");
