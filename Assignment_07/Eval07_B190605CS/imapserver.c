            ...
            else if(strcmp(data, "d") == 0)
            {
                ...
                if (!del){
                    ...
                    send_packet(C->sock_fd, res, strlen(res));
                          ...
                          char *m1, *m2,*re1, *re2,*re3, *re4;
                          int q = recieve_packet(C->sock_fd, &m1);
                          
                          if((!verify_format(m1)) && (verify_reciepent(m1, C->D, 3) == -1)){
                              re1 = "INVALID EMAIL";
                              send_packet(C->sock_fd, re1, strlen(re1)); 
                              int r = recieve_packet(C->sock_fd, &m2);
                              if((!verify_format(m2)) && (verify_reciepent(m2, C->D, 3)) == -1){
                                   send_packet(C->sock_fd, re1, strlen(re1));
                                   break; 
                              }
                              else{
                                   re4 = "OK";
                                   send_packet(C->sock_fd, re4, strlen(re4));
                                   char mailbox_file[100];
                          int c=0, a = 1,len = 0, temp = 0;
                          ...
                          while(mailbox && fscanf(mailbox, "%[^\n]%*c", line)!= EOF){
                              ...
                              if(temp && begin_with(line, "Subject")){
                                  strcpy(subject, line+9);
                                  c++;
                              }
                              if(temp && begin_with(line, "Recieved")){
                                  strcpy(time, line+10);
                                  c++;
                              }
                              if(temp && begin_with(line, "Body")){
                                  strcpy(time, line+6);
                                  c++;
                              }
                              if(temp && c == 4){
                                  len+=sprintf(buffer+len, "%d | %s | %s | %s | %s \n", a, from, time, subject, body);
                                   c = 0;
                                   a++;
                              }
                              if (strcmp(line, ".") == 0){
                            ...
                            if(len == 0)
                                len = sprintf(buffer, "NO MAILS TO SHOW\n");
                            ...
                            int p = send_packet(C->sock_fd, buffer, len); 
                             ...
                             re3 = "OK";
                             send_packet(C->sock_fd, re3, strlen(re3));
                          char mailbox_file[100];
                          int c=0, a = 1,len = 0, temp = 0;
                          sprintf(mailbox_file, "%s/mymailbox.mail", C->uname);
                          FILE *mailbox = fopen(mailbox_file, "r"); 
...
int main(int argc, char *argv[]){
   ...
   printf("Initiating IMAP Mail server.\n");
